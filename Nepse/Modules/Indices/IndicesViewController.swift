...
class IndicesViewController: UIViewController {
    ...
    private func setupSummary() {
        if let marketData = data.marketSummary {
           self.turnOverLabel.text  = marketData.currentIndex
            self.tradedSharesLabel.text = marketData.totalTradedShares
            self.transactionLabel.text = marketData.totalTransactions
            self.scriptsTradedLabel.text = marketData.totalScriptsTraded
            self.totalMarketLabel.text = marketData.pointChange
            self.floatedMarketLabel.text = marketData.percentChange
        }
    }
