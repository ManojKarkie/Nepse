...
class CompanyViewController: UIViewController, IndicatorInfoProvider {
    ...
    private func fetchSubIndexList() {
        service.fetchIndexList { (data) in
            if let array = data.companies {
                self.codeArray = array.map{$0.name ?? ""}
...
extension CompanyViewController: UIPickerViewDataSource {
...
extension CompanyViewController: UIPickerViewDelegate {
